...
function frame(m)
	...
	spec = ParticleLife.find_species(m, grid_size=grid_size, density_threshold=density_threshold, vel_threshold=vel_threshold)
	GLMakie.heatmap!(ax, 0..m.WORLD_SIZE, 0..m.WORLD_SIZE, spec, alpha=1)
	
	i=[1,2,3,4,5,6,7,8]
	scatter!(ax, m.cpu_px, m.cpu_py, color=map(x->i[x],ParticleLife.get_ptypes(m)), colormap=:curl, markersize=3)
