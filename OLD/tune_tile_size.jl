...
function patch_tile_size(src::String, tile::Int)::String
    return replace(src, r"^const TILE_SIZE = \d+"m => "const TILE_SIZE = $tile")
end
...
for tile in CANDIDATES
    ...
    try
        ...
        model = Base.invokelatest(getfield(mod, :create_model); num_particles=NUM_PARTICLES)
        ...
        @printf(
            "  median %.3f ms  |  std %.3f ms  |  min %.3f ms  |  max %.3f ms\n",
            med,
            sd,
            minimum(times),
            maximum(times)
