...
function display(model::ParticleModel; PORT=8080, HOST="0.0.0.0", TYPE_COLORS=[colorant"#e74c3c", colorant"#3498db", colorant"#2ecc71", colorant"#f39c12", colorant"#9b59b6", colorant"#1abc9c", colorant"#e91e63", colorant"#cddc39", colorant"#00bcd4",])
  ...
  app = App() do session::Session
    ...
    heatmap_on_obs = Observable(false)
    heatmap_n_obs  = Observable(8)          # coarse grid size
    heatmap_thr_obs = Observable(10)        # threshold (particles per coarse cell)
    heatmap_mat_obs = Observable(zeros(Float32, 64, 64))  # n²×n²
    ...
    # heatmap! in Makie draws x along columns, y along rows — matches our matrix
    # layout (row 1 = y-max) when we set the axis range to (0, world_size).
    # We use NaN for zero cells so they render as transparent.
    heatmap_display = @lift begin
      m = $heatmap_mat_obs
      # Replace 0 with NaN so empty cells are transparent
      replace(m, 0.0f0 => NaN32)
    end
    heatmap_alpha = @lift($heatmap_on_obs ? 0.45f0 : 0.0f0)
    hm_overlay = heatmap!(ax_sim,
      LinRange(0f0, world_size, size(heatmap_mat_obs[], 2) + 1),
      LinRange(0f0, world_size, size(heatmap_mat_obs[], 1) + 1),
      heatmap_display;
      colormap = :hot, lowclip = :transparent,
    )
    # Drive transparency via the plot's alpha attribute
    on(heatmap_alpha) do a
      hm_overlay.alpha = a
    end
    ...
    sl_hm_n   = Slider(3:16; value=heatmap_n_obs[])
    sl_hm_thr = Slider(1:200; value=heatmap_thr_obs[])
    ...
    hm_n_display   = Observable(string(heatmap_n_obs[]))
    hm_thr_display = Observable(string(heatmap_thr_obs[]))
    ...
    on(btn_heatmap) do _
      heatmap_on_obs[] = !heatmap_on_obs[]
      btn_heatmap.content[] = heatmap_on_obs[] ? "Density Map: ON" : "Density Map: OFF"
    end
      ...
      while true
        try
          ...
          if is_running[]
            ...
            if heatmap_on_obs[]
              hm = heatmap(m, heatmap_n_obs[], heatmap_thr_obs[])
              # Resize the axis ranges to match current matrix dimensions
              N = size(hm, 1)
              hm_overlay[1] = LinRange(0f0, world_size, N + 1)
              hm_overlay[2] = LinRange(0f0, world_size, N + 1)
              heatmap_mat_obs[] = hm
            end
          end
    ...
    controls = DOM.div(
      ...
      DOM.div(
        btn_heatmap,
        lbl(" Grid n: "), sl_hm_n, val(hm_n_display),
        lbl("Threshold: "), sl_hm_thr, val(hm_thr_display);
        style="display:flex; align-items:center; gap:6px; padding:4px 0;",
      );
