...
const SPECIES_DENSITY_FLOOR =
    Float32(SPECIES_HM_THRESHOLD) / Float32(SPECIES_N) / 5000.0f0 * 4.0f0
...
function find_species(
    ...
    for x0 in 1:N, y0 in 1:N
        ...
        while head <= length(queue)
            ...
            for (nx, ny) in ((qx - 1, qy), (qx + 1, qy), (qx, qy - 1), (qx, qy + 1))
                ...
                (!fine_mask[nx, ny] || hm[nx, ny] == 0.0f0 || labels[nx, ny] != 0) &&
                    continue
                labels[nx, ny] = lbl
                push!(queue, (nx, ny))
            end
    ...
    for k in 1:np
        ...
        lbl = labels[xi, yi]
        lbl == 0 && continue
        vxk = Float64(cpu_vx[k])
        vyk = Float64(cpu_vy[k])
        vx_sum[lbl] += vxk
        vy_sum[lbl] += vyk
        vx2_sum[lbl] += vxk * vxk
        vy2_sum[lbl] += vyk * vyk
...
function cluster_bounds(labels::Matrix{Int32})
    N = size(labels, 1)
    bounds = Dict{
        Int32,@NamedTuple{x_min::Int, x_max::Int, y_min::Int, y_max::Int, cells::Int}
    }()
    for x in 1:N, y in 1:N
        lbl = labels[x, y]
        lbl == 0 && continue
