...
function make_sim_axis(fig, world_size)
    ...
    xlims!(ax, 0.0f0, world_size)
    ylims!(ax, 0.0f0, world_size)
...
function setup_drag_zoom!(ax, rect_overlay)
    ...
    on(events(ax.scene).mouseposition) do _
        ...
        rect_overlay[] = Point2f[
            start, Point2f(finish[1], start[2]), finish, Point2f(start[1], finish[2]), start
        ]
    end
    on(events(ax.scene).mousebutton) do event
        event.button == Mouse.left || return nothing
        if event.action == Mouse.press
            mp = last_mouse_pos[]
            fl = ax.finallimits[]
            in_axis =
                fl.origin[1] <= mp[1] <= fl.origin[1] + fl.widths[1] &&
        ...
        elseif event.action == Mouse.release
            ...
            (
                abs(finish[1] - start[1]) < min_drag || abs(finish[2] - start[2]) < min_drag
            ) && return nothing
...
function setup_heatmap_overlay!(ax, dm, world_size)
    ...
    hm_x = Observable(LinRange(0.0f0, world_size, size(dm.heatmap_mat_obs[], 1) + 1))
    hm_y = Observable(LinRange(0.0f0, world_size, size(dm.heatmap_mat_obs[], 2) + 1))
    on(dm.heatmap_mat_obs) do m
        r = LinRange(0.0f0, world_size, size(m, 1) + 1)
        hm_x[] = r
        hm_y[] = r
    end
...
function setup_species_overlay!(ax, dm, world_size)
    ...
    return img_plot.alpha = sp_alpha[]
end
...
function setup_circle_overlay!(ax, dm, world_size)
    cx = Float32(world_size) * 0.5f0
    cy = cx
    circle_xy = @lift begin
        r = $(dm.radius_obs)
        ts = LinRange(0.0f0, 2.0f0 * Float32(π), 129)
        Point2f.(cx .+ r .* cos.(ts), cy .+ r .* sin.(ts))
    end
    circle_color = @lift($(dm.circle_vis_obs) ? RGBAf(1, 1, 1, 0.45) : RGBAf(0, 0, 0, 0))
    return lines!(ax, circle_xy; color=circle_color, linewidth=1.5, linestyle=:dash)
end
...
function setup_slider_handlers!(
    dm,
    model_ref,
    MAX_TYPES,
    MAX_WEIGHT_STEPS,
    ...
    on(sl_hm_thr.value) do v
        dm.heatmap_thr_obs[] = v
        hm_thr_display[] = string(v)
    end
    on(sl_types.value) do nt
        ...
        for t in 1:MAX_TYPES
            dm.species_weight_obs[t][] = t <= nt ? eq : 0
        end
...
function build_matrix_panel(dm, model_ref, TYPE_COLORS, MAX_TYPES, make_display_mat_fn)
    ...
    ax_mat = Axis(
        ...
        xticksvisible=false,
        yticksvisible=false,
        limits=(0.0f0, Float32(MAX_TYPES) + 0.5f0, 0.0f0, Float32(MAX_TYPES) + 0.5f0),
    )
    heatmap!(
        ax_mat,
        1:MAX_TYPES,
    ...
    on(events(ax_mat.scene).mousebutton) do event
        event.action == Mouse.press || return nothing
        ...
        if 1 <= i <= nt && 1 <= j <= nt
            ...
            m.attraction_matrix[i, j] = clamp(
                m.attraction_matrix[i, j] + delta, -1.0f0, 1.0f0
            )
            m.attr_dirty = true
            dm.mat_obs[] = make_display_mat_fn(m)
        end
...
function build_weight_sliders(dm, weight_pct_obs, TYPE_COLORS, MAX_TYPES, MAX_WEIGHT_STEPS)
    function make_one(t)
        ...
        on(sl.value) do v
            dm.species_weight_obs[t][] = v
        end
        return DOM.div(
            DOM.span("T$t"; style="color:$col; font-size:10px; font-weight:bold;"),
            sl,
            DOM.span(
                pct; style="color:#888; font-size:9px; min-width:24px; text-align:center;"
            );
            style=vis,
        )
    end
        ...
        while true
            try
                ...
                if dm.is_running[]
                    ...
                    new_hm = if dm.heatmap_on_obs[]
                        heatmap(m, dm.heatmap_n_obs[], dm.heatmap_thr_obs[])
                    else
                        nothing
                    end
                    new_species_hm =
                        dm.species_on_obs[] ? species_cluster_heatmap(model_ref[]) : nothing
                    lock(stage_lock) do
                        copy!(stage_px, m.cpu_px)
                        copy!(stage_py, m.cpu_py)
...
function display(
    ...
    app = App() do session::Session
        ...
        sl_radius = Slider(
            1:200; value=argmin(abs.(collect(radius_vals) .- model_ref[].max_radius))
        )
        sl_dt = Slider(1:200; value=argmin(abs.(collect(dt_vals) .- model_ref[].dt)))
        sl_friction = Slider(
            1:200; value=argmin(abs.(collect(friction_vals) .- model_ref[].friction))
        )
        ...
        friction_display = Observable(
            string(round(friction_vals[sl_friction.value[]]; digits=3))
        )
        ...
        on(btn_heatmap) do _
            ...
            btn_heatmap.content[] =
                dm.heatmap_on_obs[] ? "Density Map: ON" : "Density Map: OFF"
        end
        on(btn_species) do _
            ...
            btn_species.content[] =
                dm.species_on_obs[] ? "Species Map: ON" : "Species Map: OFF"
        end
        ...
        function reset_with_weights!(also_randomize=false)
            ...
            refresh_display!()
            dm.step_obs[] = "Step: 0"
            return dm.is_running[] = was
        end

        on(btn_rand) do _
            randomize_matrix!(model_ref[])
            dm.mat_obs[] = make_mat(model_ref[])
        end
        on(btn_reset) do _
            reset_with_weights!()
        end
        on(btn_both) do _
            reset_with_weights!(true)
        end
        ...
        for t in 1:MAX_TYPES
            on(dm.species_weight_obs[t]) do _
                update_pct_obs()
            end
        end
        ...
        function val(ob)
            return DOM.span(
                ob;
                style="color:#ccc; font-size:11px; min-width:38px; display:inline-block; text-align:right;",
            )
        end
        function row(c...; gap="8px")
            return DOM.div(
                c...; style="display:flex; align-items:center; gap:$gap; padding:3px 0;"
            )
        end
        function sec_title(s)
            return DOM.div(
                s;
                style="color:#ddd; font-size:11px; font-weight:bold; text-transform:uppercase; letter-spacing:0.08em; margin-bottom:4px;",
            )
        end
        ...
        tab_density_btn = DOM.span("Density"; style=tab_btn_style(:density), onclick="")   # wired via Observable below
        ...
        on(tbtn_density) do _
            overlay_tab[] = :density
        end
        on(tbtn_species) do _
            overlay_tab[] = :species
        end

        tab_density_style = @lift(
            if $overlay_tab == :density
        ...
        tab_species_style = @lift(
            if $overlay_tab == :species
                "padding:3px 12px; font-size:11px; background:#2a2a2a; color:#fff; border-bottom:2px solid #69aaff;"
            else
                "padding:3px 12px; font-size:11px; background:transparent; color:#555;"
            end
        ...
        density_body_vis = @lift(
            $overlay_tab == :density ? "display:block;" : "display:none;"
        )
        species_body_vis = @lift(
            $overlay_tab == :species ? "display:block;" : "display:none;"
        )
        ...
        cluster_legend = DOM.div(
            [
                let col = "#" * hex(CLUSTER_COLORS[i])
                    DOM.div(
                        DOM.div(;
                            style="width:10px; height:10px; border-radius:2px; background:$col; flex-shrink:0;",
                        ),
        ...
        overlay_section = DOM.div(
            ...
            DOM.div(
                ...
                DOM.div(
                    DOM.span(
                        "Coherent clusters from find_species. Each ID → unique color.";
        ...
        settings_panel = DOM.div(
            DOM.div(
                DOM.span(
                    "⚙ Settings"; style="color:#eee; font-size:13px; font-weight:bold;"
                ),
                toggle_btn;
